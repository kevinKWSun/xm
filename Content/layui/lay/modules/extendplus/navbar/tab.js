    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            ...
            element.tabAdd(ELEM.tabFilter, {
                title: title,
                content: content,
                id: globalTabIdIndex//1.0.9版本 fhua 17.2.28
            });
            //iframe 自适应
            ...
            element.tabChange(ELEM.tabFilter, globalTabIdIndex);//1.0.9版本 fhua 17.2.28
        } else {
            element.tabChange(ELEM.tabFilter, tabIndex);
            //自动刷新
            if (_config.autoRefresh) {
                $('#ifrid-' + tabIndex)[0].contentWindow.location.reload();
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').off('contextmenu');
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                //获取当前点击选项卡的索引值(id值)
                var clickIndex = $($target).attr("lay-id");//1.0.9版本 fhua 17.2.28
