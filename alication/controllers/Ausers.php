...
class Ausers extends MY_Controller {
	public function __construct() {
		...
		$this->load->model(array('user/ausers_model', 'auth/auth_model', 'gfsy/gfsy_model'));
	...
	public function solor(){
		...
		$per_page = 8;
        $offset = $current_page;
        $config['base_url'] = base_url('ausers/solor/' . $keywords);
	...
	public function add(){
		...
		$admin_id = '';
		$gs['group'] = $this->_getgroup($admin_id);
	...
	public function bind(){
		...
		$per_page = 8;
        $offset = $current_page;
        $config['base_url'] = base_url('ausers/bind/' . $keywords);
        $config['total_rows'] = $this->ausers_model->get_ausers_num($keywords, $admin_id);
        $config['per_page'] = $per_page;
		$config['page_query_string'] = FALSE;
		$config['first_link'] = '首页'; // 第一页显示   
	...
	public function sylist(){
		...
		$per_page = 8;
        $offset = $current_page;
        $config['base_url'] = base_url('user/sylist');
		...
		if ($this->agent->is_mobile('iphone')){
			$this->load->view('user/mlist', $data);
		}else{
			$this->load->view('user/list', $data);
		}
	}
	public function sysolor(){
		$keywords = $this->input->post_get('keywords', TRUE);
		if($this->input->post()){
			if(! $keywords){
				$info['state'] = 0;
		...
		$per_page = 8;
        $offset = $current_page;
        $config['base_url'] = base_url('user/sysolor');
