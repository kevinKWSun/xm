...
class Wxqd extends WX_Controller {
	public function __construct() {
        parent::__construct();
		$this->load->library('pagination');
		$this->load->helper(array('common', 'url'));
		$this->load->model(array('wx/wxqd_model', 'user/ausers_model'));
    }
	public function lists() {
		...
		$per_page = 10;
        $offset = $current_page;
        $config['base_url'] = base_url('wxqd/lists');
        $config['total_rows'] = $this->wxqd_model->get_lnum($admin_id);
        ...
        $data['p'] = $current_page;
        $lists = $this->wxqd_model->lists($per_page, $offset * $per_page, $admin_id);
		foreach($lists as $k=>$v) {
			$lists[$k]['name'] = $this->ausers_model->get_ausers_byuid($v['admin_id']);
		}
		if(! $this->input->is_ajax_request()) {
			$data['lists'] = $lists;
			$this->load->view('wx/qdlists', $data);
		} else {
			$info['html'] = '';
			if(!empty($lists)) {
				foreach($lists as $k=>$v) {
					...
					$timstamp = empty($v['addtime']) ? $timstamp : date('Y-m-d H:i', $v['addtime']);
					$info['html'] .= "<td>".$timstamp."</td>";
					$info['html'] .= "<td><a  href='".base_url('wxqd/art/'.$v['id'])."'>查看</a></td>";
					$info['html'] .= '</tr>';
				}
				$info['url'] = base_url('wxqd/lists/'.($current_page+1));
	...
	public function sign() {
		...
		if($d = $this->input->post(NULL, true)) {
			...
			$base64_image_content = $d['sign']; //匹配出图片的格式
			if(empty($d['signbak']) || empty($d['sign'])) {
				$info['state'] = 0;
				$info['message'] = '请签名！';
				$this->output
				->set_content_type('application/json', 'utf-8')
			...
			if (preg_match('/^(\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
				$type = $result[2];
				$new_file = "uploads/sign/".date('Ymd',time())."/"; 
			...
			if(!$id=$this->wxqd_model->add($insert_data)) {
				$info['state'] = 0;
				$info['message'] = '操作失败!';
				$this->output
				->set_content_type('application/json', 'utf-8')
			...
			$url = base_url('/wxqd/art/'.$id);
			$ret = $this->jssdk->signSend($ausers['authkey'], $ausers['realname'], date('Y-m-d H:i'), '签到', $url);
			$info['state'] = 1;
			$info['message'] = '操作成功！';
			$this->output
			->set_content_type('application/json', 'utf-8')
		...
		$this->load->view('wx/qdadd', $data);
	}
	/**/
	public function art() {
		$id = $this->uri->segment(3);
		$res = $this->wxqd_model->get_l($id, UID);
		$res['realname'] = $this->ausers_model->get_ausers_byuid($res['admin_id']);
		$data = array();
		$data['res'] = $res;
		$this->load->view('wx/art', $data);
	}
}