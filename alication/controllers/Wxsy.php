...
class Wxsy extends WX_Controller {
	public function __construct() {
        parent::__construct();
        $this->load->library('pagination');
		$this->load->helper(array('common', 'url'));
		$this->load->model(array('wx/wxsy_model', 'user/ausers_model'));
    }
	public function index(){
		...
		$per_page = 10;
        $offset = $current_page;
        $config['base_url'] = base_url('wxsy/index');
        $config['total_rows'] = $this->wxsy_model->get_lnum($admin_id);
        ...
        $data['p'] = $current_page;
        $lists = $this->wxsy_model->lists($per_page, $offset * $per_page, $admin_id);
		//print_r($lists);
		...
		if(!$this->input->is_ajax_request()) {
			$data['lists'] = $lists;
			$this->load->view('wx/sylists', $data);
		} else {
			$info['html'] = '';
			if(!empty($lists)) {
				foreach($lists as $k=>$v) {
	...
	public function add(){
		...
		if($d = $this->input->post(NULL, true)) { 
			...
			if(!$id=$this->wxsy_model->add($insert_data)) {
				$info['state'] = 0;
				$info['message'] = '操作失败!';
				$this->output
				->set_content_type('application/json', 'utf-8')
		...
		$this->load->view('wx/syadd', $data);
	}
	/**/
	public function art() {
		$id = $this->uri->segment(3);
		$res = $this->wxsy_model->get_l($id, UID);
		$res['realname'] = $this->ausers_model->get_ausers_byuid($res['admin_id']);
		$data = array();
		$data['res'] = $res;
		$this->load->view('wx/syart', $data);
	}
}