                            ...
                            <form id="tt" class="layui-form layui-form-pane">
                                <div class="layui-form-item">
                                    ...
                                    <div class="layui-inline">
                                        ...
                                        <a class="layui-btn layui-btn-small do-action" data-type="doRefresh" data-href="/wzcknews/lists.html">
                ...
                <form class="form-horizontal" id="formrec" method="post" role="form">
                    <table class="layui-table layui-tables" lay-skin="line">
                        ...
                        <tbody>
                            ...
                            <tr class='ck'>
								<td>
                                    <input name="ck" value="<?php echo $v['id']; ?>" title="<?php echo get_wzmc()[$v['cate']][$v['cate_name']]; ?>" cj="<?php echo get_company()[$v['cate_name']][$v['manufacturer']]; ?>" dw="<?php echo $v['unit']; ?>" ph="<?php echo $v['batch_num'] ?>" type="radio">
                                </td>
    ...
    <script>
        layui.use(['layer', 'laypage', 'common', 'form', 'paging'], function () {
            ...
            form.on('submit(cx)', function (data) {
                ...
                $.post('/wzcknews/lists.html', { keywords: keywords },
                function (result, status) {
                    if (result.state == 1) {
                        window.location.href = '/wzcknews/lists/1.html?query=' + result.message;//result.data;
                    }else{
			...
			$('tr.ck').on('click', function(){
				...
				var cj = $(this).find('input').attr('cj');
				var dw = $(this).find('input').attr('dw');
				var ph = $(this).find('input').attr('ph');
				$(window.parent.document).find('input[name=cate]').val(sid);
				$(window.parent.document).find('input[name=title]').val(title);
				$(window.parent.document).find('input[name=cj]').val(cj);
				$(window.parent.document).find('input[name=dw]').val(dw);
				$(window.parent.document).find('input[name=ph]').val(ph);
