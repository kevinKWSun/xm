                            ...
                            <form id="tt" class="layui-form layui-form-pane">
                                <div class="layui-form-item">
									<div class="layui-input-inline" style="margin-left: 0px">
										<input name="skey" lay-verify="required" value="" autocomplete="off" placeholder="领用单位名称" class="layui-input" type="text" />
									</div>
                                    <div class="layui-inline">
                                        <a class="layui-btn layui-btn-small" lay-submit="" lay-filter="cx">
                                            <i class="fa fa-search"></i>查询
                                        </a>
                                        <a class="layui-btn layui-btn-small do-action" data-type="doRefresh" data-href="/wzcknews/sylists.html">
                ...
                <form class="form-horizontal" id="formrec" method="post" role="form">
                    <table class="layui-table layui-tables" lay-skin="line">
                        ...
                        <tbody>
                            ...
                            <tr class='ck'>
								<td>
                                    <input name="ck" value="<?php echo $v['id']; ?>" title="<?php echo $v['uname']; ?>" type="radio">
                                </td>
    ...
    <script>
        layui.use(['layer', 'laypage', 'common', 'form', 'paging'], function () {
            ...
            form.on('submit(cx)', function (data) {
                ...
                $.post('/wzcknews/sylists.html', { keywords: keywords },
                function (result, status) {
                    if (result.state == 1) {
                        window.location.href = '/wzcknews/sylists/1.html?query=' + result.message;//result.data;
                    }else{
			...
			$('tr.ck').on('click', function(){
				...
				$(window.parent.document).find('input[name=leading_unit]').val(sid);
				$(window.parent.document).find('input[name=leading_name]').val(title);
