...
<body>
    ...
    layui.use(['layer', 'form'], function () {
		...
		form.on('submit(cx)', function (data) {
			...
			$.post('/yjgisnews/gis.html', { keywords: keywords },
			function (result, status) {
				if (result.state == 1) {
					window.location.href = '/yjgisnews/gis/1?query=' + result.message;//result.data;
				}else{
		...
		// 定义该矩形的显示区域
		var circle = new T.Circle(new T.LngLat(<?php echo $v['lngb'],', ',$v['latb'],'), ',$v['dradius']*1000?>,{color:"#009688",weight:2,opacity:0.5,fillColor:"red",fillOpacity:0.5,lineStyle:"solid"});
		map.addOverLay(circle);
		var circle = new T.Circle(new T.LngLat(<?php echo $v['lngb'],', ',$v['latb'],'), ',$v['qradius']*1000?>,{color:"#009688",weight:2,opacity:0.5,fillColor:"green",fillOpacity:0.5,lineStyle:"solid"});
		map.addOverLay(circle);
		var circle = new T.Circle(new T.LngLat(<?php echo $v['lngb'],', ',$v['latb'],'), ',$v['sradius']*1000?>,{color:"#009688",weight:2,opacity:0.5,fillColor:"yellow",fillOpacity:0.5,lineStyle:"solid"});
		map.addOverLay(circle);
